...
async def betterparallel(link: str, filename: str):
    ...
    for i in range(chunksize +1):
        ...
        tasks.append(parallelworker(link=link, filename=filename, startbyte=startbyte, endbyte=endbyte, progress=progress))
        startbyte = endbyte + 1
    await asyncio.gather(*tasks)
    progress.close()
    return filename, os.path.splitext(filename)[1].replace('.', '')

async def parallelworker(link:str, filename:str, startbyte: int, endbyte: int, progress):
